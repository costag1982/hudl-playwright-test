...
test.describe('User fails to login with invalid credentials: @login', () => {
  test('No details provided', async ({ loginPage, homePage, navPage, page }) => {
  ...
  test('Incorrect password', async ({ loginPage, homePage, navPage, page }) => {
    await homePage.visit();
    expect(await page.title()).toBe('Hudl: We Help Teams and Athletes Win');
  ...
  test('User sees different message after 7 failed attempts', async ({ loginPage, homePage, navPage, page }) => {
    await homePage.visit();
    expect(await page.title()).toBe('Hudl: We Help Teams and Athletes Win');
